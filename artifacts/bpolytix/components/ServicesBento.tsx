...
function Card({ service, index }: { service: typeof SERVICES[number]; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{ ...CARD_STYLE, gridArea: service.area }}
    >
      <div>
        <Icon size={24} color="#1B77F2" strokeWidth={1.75} />
        <h3
          className="mt-6"
          style={{ fontFamily: "var(--font-syne)", fontSize: "20px", color: "#F5F7FA", lineHeight: 1.2, letterSpacing: "-0.022em", fontWeight: 600 }}
        >
          {service.title}
        </h3>
      </div>
      <p
        className="mt-4"
        style={{ fontFamily: "var(--font-dm-sans)", fontSize: "15px", color: "#8892A4", lineHeight: 1.6, letterSpacing: "-0.011em" }}
      >
        {service.desc}
      </p>
    </motion.div>
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1440px] px-8">
        ...
        <h2
          ...
          style={{ fontFamily: "var(--font-syne)", fontSize: "clamp(32px, 5vw, 48px)", color: "#F5F7FA", letterSpacing: "-0.022em", lineHeight: 1.0, fontWeight: 700 }}
        >
          Eight ways we replace your overhead.
        </h2>

        {/* Desktop bento — managed entirely by the <style> below */}
        <div className="bento-desktop">
          {SERVICES.map((s, i) => (
            <Card key={s.area} service={s} index={i} />
          ))}
        </div>

        {/* Mobile stacked list */}
        <div className="bento-mobile flex flex-col gap-4">
          {SERVICES.map((s, i) => {
            const Icon = s.icon;
            return (
              <motion.div
                ...
                className="bento-card flex flex-col justify-between"
                style={{ ...CARD_STYLE, minHeight: "180px" }}
              >
                <div>
                  <Icon size={24} color="#1B77F2" strokeWidth={1.75} />
      ...
      <style jsx>{`
        /* Mobile: hide desktop bento, show stacked */
