...
| Task type | Tool |
|---|---|
| Multi-file / architectural / refactors across pages | Codex |
| New page builds, contained component edits, single-file copy/style changes | Codex (GPT-5.5 High reasoning) |
| Visual / frontend debug | Codex (load bpolytix-frontend-debugger skill first) |
| Backend / API / deploy debug | Codex (load bpolytix-backend-debugger skill first) |
| File inspection / diagnostic without code change | Either — Codex preferred for token cost |

Codex is for tasks where reasoning across multiple files matters more than throughput. Codex is for contained tasks where the brief defines the answer.
